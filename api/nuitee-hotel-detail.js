export default async function handler(req,res){
 ...
 try{
  const r=await fetch('https://api.liteapi.travel/v3.0/data/hotel?'+new URLSearchParams({hotelId,timeout:'6',language:'es'}),{headers:{'X-API-Key':key,Accept:'application/json'}});
  const j=await r.json();if(!r.ok)throw new Error(j?.error?.message||j?.message||'No se pudo cargar el hotel');
  const d=j.data||j;
